...
void GUIPanel::onMQTT_Received(const QMQTT::Message &message)
{
    ...
    if (connected)
    {
        ...
        { //Tengo que comprobar que el mensaje es del tipo adecuado y no hay errores de parseo...
            ...
            QJsonValue entrada2=objeto_json["blueLed"]; //Obtengo la entrada blueLed. Esto lo puedo hacer porque el operador [] está sobrecargado
...
void GUIPanel::SendMessage()
{
    ...
    //Añade un campo "blueLed" al objeto JSON, con el valor (true o false) contenido en checked
    objeto_json["blueLed"]=ui->pushButton_3->isChecked(); //Puedo hacer ["blueLed"] porque el operador [] está sobrecargado.
    //Añade un campo "greenLed" al objeto JSON, con el valor (true o false) contenido en checked
    objeto_json["greenLed"]=ui->pushButton_4->isChecked();
